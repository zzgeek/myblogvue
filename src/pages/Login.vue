...
<template>
    <div>
        <div class="container">
            <h1 class="loginTitle">
                <a href="#/">{{this.$store.state.themeObj.user_start!=0?"青涩知夏的博客":"青涩知夏的博客"}}</a>
            </h1>
            <!-- 登录注册 -->
            <div v-show="!err2005" class="">
                <div v-if="login==1" class="loginBox">
                    ...
                    <el-input type="email" placeholder="账号" v-model="username"></el-input>
                    <el-alert v-show="usernameErr" title="请输入账号" type="error" show-icon  :closable="false"></el-alert>
                    <el-input type="password" placeholder="密码" @keyup.enter.native="loginEnterFun" v-model="password"></el-input>
                ...
                <div v-else class="registerBox">
                    <div class="lr-title"><h1>注册</h1><p>已有账号<a href="#/Login?login=1" class="tcolors">登录</a></p></div>
                    <el-alert v-show="registerErr" :title="registerTitle" type="error" show-icon  :closable="false"></el-alert>
                    ...
                    <el-input type="password" placeholder="确认密码" @keyup.enter.native="registerEnterFun" v-model="npassword2"></el-input>
            ...
            <div v-show="err2005" class="registerSuc">
                ...
                <div v-show="urlstate==0" class="sucContent">账号激活链接已发送至您的邮箱：{{nemail}}<p>请您在24小时内登录邮箱，按邮件中的提示完成账号激活操作</p></div>
                <div v-show="urlstate=='urlInvalid'" class="sucContent">账号已激活，现在去登录 &nbsp;&nbsp;<span class="tcolors-bg lastbtn" @click="goLogin">登录</span></div>
                <div v-show="urlstate=='urlErr'" class="sucContent">OwO邮箱激活地址已超时，验证失败，请重新注册 &nbsp;&nbsp;<span class="tcolors-bg lastbtn" @click="goRegister">注册</span></div>
            </div>
        </div>
    </div>
</template>
        ...
        methods: { //事件处理器
            ...
            newRegister:function(){//注册提交
                ...
                if(!that.nusernameErr&&!that.npasswordErr){
                    ...
                    getRegister(that.nusername,that.npassword,function(msg){
                        console.log(msg);
                        if(msg.resultCode === '00000'){//注册成功
                            ...
                            that.registerTitle = msg.message
                        }
                    })
                }
            },
