				...
				<div class="headBox">
					<el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal" @select="handleSelect" :router="true">
						...
						<el-submenu index="/Share">
							...
							<el-menu-item v-for="(item,index) in categorys" :key="'class1'+index" :index="'/Share?classId='+item.id">{{item.name}}</el-menu-item>
						</el-submenu>
						...
						<div class="userInfo">
							...
							<div v-show="haslogin" class="haslogin">
								...
								<ul class="haslogin-info">
									...
									<li><a href="#/likeCollect?like=1">喜欢列表</a></li><li><a href="#/likeCollect?like=2">收藏列表</a></li>
					...
					<div class="hideMenu">
						...
						<el-collapse-transition>
							<el-menu :default-active="activeIndex" class="mlistmenu" v-show="!pMenu" theme="dark" @open="handleOpen" @close="handleClose" :unique-opened="true" :router="true">
								...
								<el-submenu index="/Share">
									<template slot="title"><i class="fa fa-wa fa-archive"></i> 分类</template>
									<el-menu-item v-for="(item,index) in categorys" :key="'class1'+index" :index="'/Share?classId='+item.class_id">{{item.cate_name}}</el-menu-item>
								</el-submenu>
<!--								<el-submenu index="2">-->
<!--									<template slot="title"><i class="fa fa-wa fa-flask"></i> 实验室</template>-->
<!--									<el-menu-item v-for="(item,index) in projectList" :key="'class2'+index" index=""><a :href="item.nav_url" target="_blank">{{item.nav_name}}</a></el-menu-item>-->
<!--								</el-submenu>-->
	...
	<div class="headImgBox" :style="{backgroundImage:this.$store.state.headBg?'url('+this.$store.state.headBg+')':'url(static/img/headbg05.jpg)'}">
		<div class="scene">
			<div><span id="luke"></span></div>
		</div>
		<div class="h-information">
			<a href="#/Aboutme">
        <img :src="this.$store.state.headPortrait?this.$store.state.headPortrait:'static/img/tou.png'" alt="">
      </a>
			<h2 class="h-description">
        <a href="#/Aboutme">{{this.$store.state.themeObj.autograph?this.$store.state.themeObj.autograph:"Write the Code. Change the World."}}</a>
      </h2>
		</div>
	</div>
</div>
...
export default {
	...
	watch: {
		'$route': 'routeChange'// 如果路由有变化，会再次执行该方法
	},
	created() { //生命周期函数
     //判断当前页面是否被隐藏
		let that = this;
		...
		this.routeChange();
		//设置主题
		// changeTheme((res)=>that.$store.state.themeObj = res);
		//关于我的信息
		// AboutMeData((res)=>that.$store.state.aboutmeObj = res);
	},
	mounted() { //页面元素加载完成
	  	let that = this;
      let timer = setTimeout(function() {
			Typeit(that.$store.state.username, "#luke"); //打字机效果
			clearTimeout(timer);
		}, 500);
	}
}
