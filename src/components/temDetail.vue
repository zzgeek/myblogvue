        ...
        <div class="detailBox tcommonBox" >
            ...
            <header>
                ...
                <h2>
                    ...
                    <i class="fa fa-fw fa-comments"></i>活捉 {{detailObj.comments}} 条 •
                    <span class="rateBox">
            ...
            <div class="article-content" v-highlight="detailObj.content"></div>
        ...
        methods: { //事件处理器
            ...
            likeCollectHandle: function(isLike){//用户点击喜欢0,用户点击收藏1
                ...
                if(localStorage.getItem('userInfo')){//判断是否登录
                    ...
                    getArtLikeCollect(that.userId,that.aid,isLike,function(res){
                        that.$message({
                             message: tip,
                             type: 'success'
                           });
            ...
            routeChange:function(){
                ...
                getArticleInfo(that.aid,function(res){
                    that.detailObj = res;
                    that.likeCount = res.likeCount?res.likeCount:0;
                    that.collectCount = res.collectCount?res.collectCount:0;
                    // that.likeArt = res.userLikeStart===0?false:true; // 判断是否点赞过文章
                    // that.collectArt = res.userCollectStart===0?false:true; // 判断是否收藏过文章
