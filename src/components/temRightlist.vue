...
<template>
  <div class="rightlistBox">
    <section>
      <div class="r1-head">
        <img :src="this.$store.state.rightPortrait?this.$store.state.rightPortrait:'static/img/headtou02.jpg'" alt="">
      </div>
      <div class="r1-body">
        <p>你能抓到我么？</p>
        <div class="catch-me">
      ...
      <ul class="rs3-textwidget">
        <li class="rs3-item" v-for="(item,index) in artCommentList" :key="'artCommentList'+index">
          <a :href="'#/DetailShare?aid='+item.id" target="_blank">
            <div class="rs3-photo"><img :src="item.authorAvatar" :onerror="$store.state.errorImg"></div>
            <div class="rs3-inner">
              <p class="rs3-author">{{item.authorName}} 在「{{item.title}}」中说:</p>
    ...
    <section class="rs4">
      ...
      <ul>
        <li v-for="(item,index) in browseList" :key="'browseList'+index">
          <a :href="'#/DetailShare?aid='+item.id" target="_blank">{{item.title}}</a> —— {{item.views}} 次围观
        </li>
      </ul>
    </section>
    <!-- 右侧上滑小图片 -->
...
<script>
    ...
    export default {
        ...
        methods: {
            lovemeFun: function () {
                ...
                let timer = setTimeout(function () {
                    that.loveme = false; //这里无法直接使用 this
                    clearTimeout(timer);
                }, 3000)
            },
            toTopfun: function (e) {
        ...
        created() { //生命周期函数
            ...
            //查询浏览量最多的10篇文章数据
            showBrowseCount((res) =>this.browseList = res);
            //查询文章评论量最大的10篇文章
